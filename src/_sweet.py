...
class ForceFirstTokenLogitsProcessor(LogitsProcessor):
    def __init__(self, tokenizer, forced_token):
        self.forced_token = forced_token
        self.forced_token_id = tokenizer.convert_tokens_to_ids(forced_token)
        self.has_turned = False
    
    def __call__(self, input_ids, scores):
        if not self.has_turned:
            scores[:, :] = float('-inf')
            scores[:, self.forced_token_id] = 0
...
class WatermarkBase:
    def __init__(
        ...
        gamma: float = 0.5,
        delta: float = 2.0,
        seeding_scheme: str = "n_grams",  # mostly unused/always default
        ngram_len: int = 5,
...
class WatermarkDetector(WatermarkBase):
    def __init__(
        ...
        **kwargs,
        ...
        if self.seeding_scheme == "n_grams":
            self.min_prefix_len = self.ngram_len - 1
        else:
