...
transform_job=$(sbatch --parsable --array=0-14%15 --output="${LOG_DIR}/transform-%A_%a.out" --error="${LOG_DIR}/transform-%A_%a.err" --export=ALL,RUN_ID="${RUN_ID}" "${SLURM_DIR}/transform_array.sbatch")
validate_job=$(sbatch --parsable --dependency=afterok:"${transform_job}" --output="${LOG_DIR}/validate-%j.out" --error="${LOG_DIR}/validate-%j.err" --export=ALL,RUN_ID="${RUN_ID}" "${SLURM_DIR}/validate_transforms.sbatch")
...
printf 'transform=%s\nvalidate=%s\nwllm=%s\nsynthid=%s\nsweet_llama=%s\nsweet_deepseek=%s\naggregate=%s\n' \
  "${transform_job}" "${validate_job}" "${wllm_job}" "${synthid_job}" "${sweet_llama_job}" "${sweet_deepseek_job}" "${aggregate_job}"
