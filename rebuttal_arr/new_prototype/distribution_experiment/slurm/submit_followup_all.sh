#!/bin/bash
set -euo pipefail
RUN_ID="${RUN_ID:-rw500-z4-sample100-v1}"
...
LOG="${RUN_ROOT}/slurm"
[[ "${RUN_ID}" == "rw500-z4-sample100-v1" ]] || { echo "unapproved run id" >&2; exit 1; }
...
mkdir -p "${LOG}"
transform=$(sbatch --parsable --output="${LOG}/transform-%A_%a.out" --error="${LOG}/transform-%A_%a.err" --export=ALL,RUN_ID="${RUN_ID}" "${SLURM}/transform_followup_all.sbatch")
...
strict=$(sbatch --parsable --dependency=afterok:"${aggregate}" --output="${LOG}/strict-%j.out" --error="${LOG}/strict-%j.err" --export=ALL,RUN_ID="${RUN_ID}" "${SLURM}/strict_ad.sbatch")
printf 'transform=%s\nvalidate=%s\nwllm=%s\nsweet=%s\nsynthid=%s\naggregate=%s\nstrict=%s\n' "${transform}" "${validate}" "${wllm}" "${sweet}" "${synthid}" "${aggregate}" "${strict}"
